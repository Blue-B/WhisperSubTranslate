...
将视频音频转成 SRT 字幕，并翻译到目标语言的 Windows 桌面应用。提取使用 whisper.cpp 快速稳定处理，翻译可选择 **本地模型（Hy-MT2 1.8B/7B GGUF，完全离线）** 或在线引擎（MyMemory 免费、DeepL、OpenAI GPT-5.4 mini、Gemini）。
...
- 从 Releases 下载最新便携版压缩包：`WhisperSubTranslate-v2.2.0-win-x64.zip`
...
- 需要 **Node.js ≥ 20.19 或 ≥ 22.12**（Electron 42 构建工具链）
...
| 翻译（可选） | 本地（Hy-MT2 1.8B/7B GGUF，node-llama-cpp）、DeepL API、OpenAI（GPT-5.4 mini）、Gemini、MyMemory |

## 翻译引擎

WhisperSubTranslate 可使用内置的腾讯 **Hy-MT2** 模型**完全离线**翻译字幕，也可使用自己的 API 密钥调用免费/付费在线引擎。

| 引擎                           | 离线 |  密钥  | 费用           | 说明                                                                                              |
| ------------------------------ | :--: | :----: | -------------- | ------------------------------------------------------------------------------------------------- |
| **Hy-MT2 1.8B**（本地 · 默认） |  ✅  | 不需要 | 免费           | 约 1.13GB 模型，VRAM 2GB / RAM 4GB，快速                                                          |
| **Hy-MT2 7B**（本地）          |  ✅  | 不需要 | 免费           | 约 6.16GB 模型，VRAM 8GB / RAM 12GB，高质量                                                       |
| MyMemory                       |  ❌  | 不需要 | 免费           | 每 IP 约 5 万字/日                                                                                |
| DeepL                          |  ❌  |  需要  | 每月 50 万免费 | 有付费档                                                                                          |
| OpenAI GPT-5.4 mini | ❌ | 需要 | 付费 | 输入 $0.075 / 输出 $0.60 per 1M tokens · 上下文感知 |
| OpenAI GPT-5.4 nano | ❌ | 需要 | 付费 | 更便宜的档位 — 输入 $0.20 / 输出 $1.25 per 1M tokens |
| Gemini 3 Flash                 |  ❌  |  需要  | 免费/付费      | 免费: 每日250字幕/20-30分钟，付费: 无限制 ([获取API密钥](https://aistudio.google.com/app/apikey)) |

> **隐私：** 本地 Hy-MT2 引擎是唯一无需 API 密钥、无需联网、无按次费用的选项 — 您的视频及其对白绝不离开本机。
>
> **提示**: 1小时以上的长视频可能会触发MyMemory的日限额，建议使用Gemini或DeepL。

### 翻译质量（离线引擎）

WhisperSubTranslate 内置腾讯 **Hy-MT2** 模型（默认 1.8B，可选 7B）。在腾讯官方多基准评测中，Hy-MT2 系列与主流商用翻译 API 相当，并在多个基准上领先：

![腾讯 Hy-MT2 官方基准 (WhisperSubTranslate 内置模型)](../assets/hy-mt2-benchmark.zh.png)

> **来源：** 由模型作者腾讯发布的官方基准 — [Hy-MT2 仓库](https://github.com/Tencent-Hunyuan/Hy-MT2) · [技术报告](https://arxiv.org/pdf/2605.22064) · [HuggingFace 模型](https://huggingface.co/tencent/Hy-MT2-1.8B)。上图为**重绘自腾讯官方 Figure 1**，内置模型(1.8B/7B)数值已与论文表格核对。这些数据是在标准机器翻译基准（WildMTBench、WMT25、FLORES-200 等）上对**模型本身**的测量，**并非**对 WhisperSubTranslate 应用的重新基准测试。据腾讯称，7B/30B 超过 DeepSeek-V4-Pro/Kimi 等开源模型，轻量级 **1.8B（默认）** 在整体上也超过 Microsoft、Doubao 等主流商用 API。
...
单一主干：`main` 作为唯一的长期分支，维护者直接在 `main` 上提交和打标签（如 `v2.2.0`）。
