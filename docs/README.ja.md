...
動画の音声を文字起こし(SRT)し、希望の言語に翻訳する Windows デスクトップアプリ。抽出は whisper.cpp で高速かつ安定して処理され、翻訳は **ローカルモデル(Hy-MT2 1.8B/7B GGUF、完全オフライン)** またはオンラインエンジン(MyMemory 無料、DeepL、GPT-5.4 mini、Gemini)から選択できます。
...
- Releases から最新のポータブルアーカイブをダウンロード:`WhisperSubTranslate-v2.2.0-win-x64.zip`
...
- **Node.js ≥ 20.19 または ≥ 22.12** が必要 (Electron 42 ビルドツールチェーン)
- **whisper-cpp**は`npm install`時に自動ダウンロードされます(Windows: ~700MB CUDA版)
- **Linux/macOS**では、ビルド済みバイナリがない場合、ソースから自動ビルドされます(`cmake`、`gcc`/`clang`、`git`が必要)
...
WhisperSubTranslateはいくつかの追加手順でLinuxでも動作します:
...
# CUDA GPUアクセラレーション(オプション、NVIDIA GPU + ドライバーが必要)
# CUDA Toolkitインストール: https://developer.nvidia.com/cuda-downloads
```
...
npm install    # whisper.cppがソースから自動ビルドされます
npm start
```

自動ビルドに失敗した場合、whisper.cppを手動でビルドしてください:
...
| 翻訳(任意)     | ローカル(Hy-MT2 1.8B/7B GGUF、node-llama-cpp), DeepL API, OpenAI(GPT-5.4 mini), Gemini, MyMemory |

## 翻訳エンジン

WhisperSubTranslate は、バンドルされた Tencent **Hy-MT2** モデルで字幕を**完全オフライン**で翻訳するか、自分の API キーで無料/有料のオンラインエンジンを利用できます。

| エンジン                          | オフライン | API キー | コスト      | 備考                                                                                           |
| --------------------------------- | :--------: | :------: | ----------- | ---------------------------------------------------------------------------------------------- |
| **Hy-MT2 1.8B**（ローカル・既定） |     ✅     |   不要   | 無料        | 約 1.13GB モデル、VRAM 2GB / RAM 4GB、高速                                                     |
| **Hy-MT2 7B**（ローカル）         |     ✅     |   不要   | 無料        | 約 6.16GB モデル、VRAM 8GB / RAM 12GB、高品質                                                  |
| MyMemory                          |     ❌     |   不要   | 無料        | 1 IP あたり約 5万/日                                                                           |
| DeepL                             |     ❌     |   必要   | 月 50万無料 | 有料プランあり                                                                                 |
| OpenAI GPT-5.4 mini | ❌ | 必要 | 有料 | 入力 $0.075 / 出力 $0.60 per 1M トークン · 文脈考慮 |
| OpenAI GPT-5.4 nano | ❌ | 必要 | 有料 | より安価なティア — 入力 $0.20 / 出力 $1.25 per 1M トークン |
| Gemini 3 Flash                    |     ❌     |   必要   | 無料/有料   | 無料: 1日250字幕/20-30分、有料: 無制限 ([APIキー取得](https://aistudio.google.com/app/apikey)) |

> **プライバシー:** ローカル Hy-MT2 エンジンは、APIキー・ネットワーク・従量課金が一切不要な唯一の選択肢です — 動画とその台詞がPCの外に出ることはありません。
>
> **ヒント**: 1時間以上の長い動画では、MyMemoryの日次制限に達することがあります。GeminiかDeepLの利用をおすすめします。

### 翻訳品質（オフラインエンジン）

WhisperSubTranslate は Tencent の **Hy-MT2** モデル（既定 1.8B、オプション 7B）をバンドルしています。Tencent 公式のマルチベンチマーク評価では、Hy-MT2 ファミリーは主要な商用翻訳 API と競合し、一部のベンチマークではそれらを上回ります：

![Tencent Hy-MT2 公式ベンチマーク (WhisperSubTranslate 同梱モデル)](../assets/hy-mt2-benchmark.ja.png)

> **出典:** モデル開発元 Tencent が公開した公式ベンチマーク — [Hy-MT2 リポジトリ](https://github.com/Tencent-Hunyuan/Hy-MT2) · [技術レポート](https://arxiv.org/pdf/2605.22064) · [HuggingFace モデル](https://huggingface.co/tencent/Hy-MT2-1.8B)。上のグラフは **Tencent 公式 Figure 1 を再描画**したもので、同梱モデル(1.8B/7B)の数値は論文の表と照合済みです。これらの数値は標準的な機械翻訳ベンチマーク（WildMTBench、WMT25、FLORES-200 など）で**モデル自体**を測定したものであり、WhisperSubTranslate アプリを再測定したものでは**ありません**。Tencent によると、7B/30B は DeepSeek-V4-Pro/Kimi などのオープンモデルを上回り、軽量な **1.8B（既定）** も Microsoft・Doubao などの主要な商用 API を総合的に上回ります。
...
| エラーログ (Windows) | `%APPDATA%\whispersubtranslate\logs\errors.log`                     |
| エラーログ (macOS)   | `~/Library/Application Support/whispersubtranslate/logs/errors.log` |
| エラーログ (Linux)   | `~/.config/whispersubtranslate/logs/errors.log`                     |
| ジョブ履歴           | `%APPDATA%\whispersubtranslate\history.json` (最大200件まで保存)    |
| モデル               | `_models/`(アプリフォルダ内)                                        |

### ジョブ履歴

- 完了したジョブを自動記録 — **最大200件**まで。
- 各行に**開く**（結果ファイルを再生）と**フォルダ**（エクスプローラーで表示）ボタン。
- **設定 → 履歴**でいつでもON/OFF。OFFにしても新規記録のみ停止され、既存データは保持されます。
- **全てをクリア**時はフォレンジック安全な抹消（ゼロ上書き、削除＋localStorageのpaddingコンパクション）。SSDのwear levelingのためSWだけで100%侩復不可を保証できません。

### モデルダウンロードの安全性

- ダウンロード中はカードに**キャンセル**ボタンが表示され、いつでも中断できます。
- ダウンロード中にウィンドウを閉じても安全に中断されます。
- Whisper GGMLファイルは`.partial`一時ファイルに受け、**完了時のみ**`ggml-*.bin`にrenameされます — 部分ファイルが「インストール済み」と誤誊されることはありません。
...
単一トランク：`main` のみを長寿ブランチとして運用し、メンテナーは `main` に直接コミット・タグ付けします（例: `v2.2.0`）。

**外部コントリビューター** は、フォークから Pull Request を作成してください。`feature/<scope>` 形式の短命ブランチを推奨し、squash マージで `main` に取り込みます。
